...
func GetSimpleLogs(c *gin.Context) {
	...
	if userFilter != "" {
		...
		if err != nil {
			...
			user := types.User{}
			err = database.Database.Model(&types.User{}).Where("username = ?", userFilter).Find(&user).Error
			if err != nil {
				c.JSON(400, gin.H{
					"success": false,
	...
	baseQuery := database.Database.Model(&types.SimpleLog{})
